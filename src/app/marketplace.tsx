...
export default function Marketplace({ products }: { products: AffiliateProduct[] }) {
  ...
  return <main>
    <div className={s.notice}>INDEPENDENT PRODUCT DISCOVERY <span>SHOP FROM TRUSTED RETAILERS</span></div>
    ...
    <section className={s.hero} id="top"><div className={s.heroCopy}><small>● CURATED FOR PACIFIC SHOPPERS</small><h1>Great finds.<br /><em>One Pacific destination.</em></h1><p>Discover useful products from Amazon, eBay and AliExpress. Compare our curated picks, then buy securely from the retailer.</p><div className={s.actions}><button onClick={() => document.getElementById("shop")?.scrollIntoView({ behavior: "smooth" })}>Explore products →</button><Link href="/affiliate-disclosure">How affiliate links work</Link></div><div className={s.trust}><span>✓ Retailer checkout</span><span>◇ Curated product links</span><span>★ No extra cost to you</span></div></div>
      <div className={s.heroVisual}><div className={s.largePhoto}><Image src="https://images.unsplash.com/photo-1580458148390-76414895c3cd?auto=format&fit=crop&w=1200&q=90" alt="Turquoise ocean waves meeting a Pacific beach" fill priority sizes="(max-width:800px) 100vw,50vw" /></div><div className={s.perk}><small>SHOP YOUR WAY</small><strong>3</strong><p>leading marketplaces in one place</p><code>AMAZON · EBAY · ALIEXPRESS</code></div></div></section>
    <section className={s.shop} id="shop"><div className={s.sectionHead}><div><small>CURATED MARKETPLACE</small><h2>{category === "All" ? "Popular product discoveries" : category}</h2></div><p>{shown.length} products</p></div>
      ...
      : <div className={s.empty}><h3>Our first product picks are coming soon</h3><p>We are preparing approved links from Amazon, eBay and AliExpress. Check back shortly.</p></div>}</section>
    <section className={s.editorial}><div><small>HOW IT WORKS</small><h2>Discover here. Buy from the retailer.</h2></div><ol><li><b>1</b><span><strong>Browse curated picks</strong>Find useful products selected for Pacific shoppers.</span></li><li><b>2</b><span><strong>Open the retailer</strong>Prices and availability are confirmed on the retailer’s website.</span></li><li><b>3</b><span><strong>Checkout securely</strong>The retailer handles payment, shipping, returns and support.</span></li></ol></section>
