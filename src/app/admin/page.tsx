...
export default async function AdminPage() {
  ...
  return <main className={styles.dashboard}>
    ...
    <section className={styles.stats}><article><span>Affiliate products</span><strong>{products?.length ?? 0}</strong></article><article><span>Active listings</span><strong>{products?.filter((p) => p.is_active).length ?? 0}</strong></article><article><span>Retailer clicks</span><strong>{affiliateClicks ?? 0}</strong></article><article><span>Future direct sales</span><strong>{money(paidTotal)}</strong></article></section>
    <section className={styles.panel}><div className={styles.panelHeading}><div><p className={styles.eyebrow}>AFFILIATE CATALOGUE</p><h2>Add a retailer product</h2></div></div><form action={createProduct} className={styles.productForm}>
      <label>Product name<input name="name" required /></label><label>URL slug<input name="slug" placeholder="wireless-headphones" required /></label><label>Retailer<select name="retailer" required><option>Amazon</option><option>eBay</option><option>AliExpress</option></select></label><label>Category<select name="category_id"><option value="">Uncategorised</option>{categories?.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
      <label>Current price (AUD)<input name="price_aud" type="number" min="0" step="0.01" required /></label><label>Original price (optional)<input name="original_price_aud" type="number" min="0" step="0.01" /></label><label>Rating (optional)<input name="rating" type="number" min="0" max="5" step="0.1" /></label><label>Review count<input name="review_count" type="number" min="0" step="1" defaultValue="0" required /></label>
    ...
    <section className={styles.panel}><div className={styles.panelHeading}><div><p className={styles.eyebrow}>CATALOGUE</p><h2>Affiliate products</h2></div><span>{products?.length ?? 0} total</span></div><div className={styles.tableWrap}><table><thead><tr><th>Product</th><th>Retailer</th><th>Price</th><th>Status</th><th>Action</th></tr></thead><tbody>
      {products?.map((p) => <tr key={p.id}><td><strong>{p.name}</strong><small>/{p.slug}</small></td><td>{p.retailer ?? "—"}</td><td>{money(p.price_aud_cents)}</td><td><span className={p.is_active ? styles.live : styles.draft}>{p.is_active ? "Live" : "Draft"}</span></td><td><form action={toggleProduct}><input type="hidden" name="id" value={p.id} /><input type="hidden" name="active" value={String(!p.is_active)} /><button className={styles.textButton}>{p.is_active ? "Unpublish" : "Publish"}</button></form></td></tr>)}
